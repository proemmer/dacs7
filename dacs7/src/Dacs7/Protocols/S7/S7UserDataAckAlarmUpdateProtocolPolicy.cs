    ...
    public class S7UserDataAckAlarmUpdateProtocolPolicy : S7UserDataProtocolPolicy
    {
        ...
        public override void SetupMessageAttributes(IMessage message)
        {
            ...
            if (length > 6)
            {
                ...
                var subItemExtended = string.Format("Alarm[{0}].ExtendedData[{1}].", 0, 0) + "{0}";
                var offset = 0;
                message.SetAttribute(string.Format(subItemExtended, "Timestamp"), sslData.ToDateTime(offset));
                var item1 = sslData.GetSwap<UInt16>(offset += 8);
                var item2 = sslData.GetSwap<UInt16>(offset += 2);
                var item3 = sslData.GetSwap<UInt16>(offset += 2);
