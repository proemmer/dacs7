    ...
    public class ReadWriteIEnumerableValuesTests
    {
        ...
        public async Task ReadWriteSingleBits()
        {
            await PlcTestServer.ExecuteClientAsync(async (client) =>
            {
                ...
                ItemResponseRetValue[] writeResults = (await client.WriteAsync(originWriteTags)).ToArray();


                DataValue[] results = (await client.ReadAsync(originWriteTags.Keys)).ToArray();
        ...
        public async Task WriteBigDBData()
        {
            await PlcTestServer.ExecuteClientAsync(async (client) =>
            {
                byte[] data = Enumerable.Repeat((byte)0x00, 15450).ToArray();
                Dictionary<string, object> originWriteTags = new()
                {
